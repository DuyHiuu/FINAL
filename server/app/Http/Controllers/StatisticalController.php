...
class StatisticalController extends Controller
{
    ...
    public function get_top3_room(Request $request)
    {
        ...
        if (count($result) == 0) {
            if ($data['timeline'] == 'year') {
                return response()->json(["message" => "Không có dữ liệu thống kê top 3 phòng  có doanh thu cao nhất năm " . $data['year']]);
            } else {
                return response()->json(["message" => "Không có dữ liệu thống kê top 3 phòng có doanh thu cao nhất tháng " . $data['month'] . " năm " . $data['year']]);
            }
        } else {
